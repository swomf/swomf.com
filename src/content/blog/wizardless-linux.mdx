...
- [Swap](https://wiki.archlinux.org/title/Swap) partition or file _(optional)_:
  ...
  - [Reddit: Why use swap?](https://old.reddit.com/r/archlinux/comments/ro4zme/is_swap_really_necessary_anymore_for_desktop/)
...
<details class="p-2 border-2 border-[var(--white)] rounded-lg">

<summary class="m-1"> Why `--xattrs-include='*.*'`? </summary>

Preserving all extended attrs (such as the setUID bit) are important for security software such
as Pluggable Authentication Module. It is the difference between being
able to login during a lockscreen:
...
This idea is taken from the [Gentoo § The stage file § Installing a stage file](https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Stage#Installing_a_stage_file) and should be used when directly
unpacking a rootfs onto a mountpoint
...
- If you untar'd a rootfs earlier, update your system.
  ...
  <span class="text-xs">
    (command sources:
    [Arch](https://wiki.archlinux.org/title/Install_Arch_Linux_from_existing_Linux#Initializing_pacman_keyring),
...
- Build confs
  ...
  - Gentoo (/etc/portage/make.conf)
    ...
    - Optional: [ACCEPT_LICENSE](https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Base#Optional:_Configure_the_ACCEPT_LICENSE_variable)
...
- [Arch §3.8](https://wiki.archlinux.org/title/Installation_guide#Boot_loader) preparing to boot
  - Gentoo OpenRC guys need:
    a [logger](https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Tools#OpenRC) (e.g. sysklogd)
  - Get programs relating to your filesystems, e.g. btrfs-progs.
    See [Gentoo § Tools § Filesystem tools](https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Tools#Filesystem_tools)
  - and <tooltip>install a bootloader<tip-after>UEFI or MBR firmware load a tiny binary that loads a bootloader (GRUB) that finds out how to shove an initramfs (a tiny Linux) into RAM. Then, the initramfs mounts your storage stuff via fstab, and switches root to it, among other things.</tip-after></tooltip>.
    See [ArchWiki: GRUB § Installation](https://wiki.archlinux.org/title/GRUB#Installation) and
    [Gentoo § Bootloader](https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Bootloader#Default:_GRUB)
    ...
    - Arch Linux mkinitcpio+GRUB example:
      - [Arch Linux by default uses BusyBox, though it can use a systemd initramfs as well (not necessarily faster)](https://wiki.archlinux.org/title/Improving_performance/Boot_process#Using_systemd_instead_of_busybox_on_early_init).
        With [mkinitcpio](https://wiki.archlinux.org/title/Mkinitcpio), you can easily switch to systemd by flipping [these bold words (with or without "encrypt")](https://wiki.archlinux.org/title/Dm-crypt/Encrypting_an_entire_system#Configuring_mkinitcpio).
      - The Arch docs are straightforward as long as you don't
        incorrectly copy a UUID:
        - LUKS?
          - [Specify encryption](https://wiki.archlinux.org/title/Dm-crypt/Encrypting_an_entire_system#Configuring_mkinitcpio) in /etc/mkinitcpio.conf [HOOKS](https://wiki.archlinux.org/title/Mkinitcpio#Common_hooks)
            - If you used LVM, [specify lvm2 as well](https://wiki.archlinux.org/title/Dm-crypt/Encrypting_an_entire_system#Configuring_mkinitcpio_6)
          - then add kernel parameters ([cryptdevice or rd.luks.name](https://wiki.archlinux.org/title/Dm-crypt/Encrypting_an_entire_system#Configuring_the_boot_loader),
            [and maybe swap](https://wiki.archlinux.org/title/Dm-crypt/System_configuration#resume)), as well as [GRUB cryptomounting ability](https://wiki.archlinux.org/title/Dm-crypt/Encrypting_an_entire_system#Configuring_GRUB_2)
            ```bash
            # /etc/default/grub (busybox example)
            GRUB_ENABLE_CRYPTODISK=y
            GRUB_CMDLINE_LINUX="cryptdevice=UUID=<use lsblk -o name,uuid>:cryptroot"
            ```
            - Pick the _LUKS_ partition ID, not the cryptroot that LUKS mapped to,
              as we're telling the bootloader to cryptomount /dev/sda2 or whichever
              to /dev/mapper/cryptroot here.
              And again, GRUB auto-specifies where root= will be. [source](https://wiki.archlinux.org/title/Dm-crypt/System_configuration#Kernel_parameters)
          - If /boot is inside the LUKS partition, add a keyfile into the initramfs. See
            [ArchWiki: Dm-crypt/Device encryption § With a keyfile embedded in the initramfs](https://wiki.archlinux.org/title/Dm-crypt/Device_encryption#With_a_keyfile_embedded_in_the_initramfs).
            - As mentioned, GRUB
              doesn't support argon2id yet (code architecturing reasons), so either use pbkdf2
              ```bash
              ... (previous 3 cmds)
              ...
              ```
              or [use a GRUB patch](https://aur.archlinux.org/packages/grub-improved-luks2-git).
        - [Generate the initramfs](https://wiki.archlinux.org/title/Mkinitcpio#Manual_generation) and [install GRUB](https://wiki.archlinux.org/title/GRUB#Installation) ([perhaps for MBR](<https://wiki.archlinux.org/title/GRUB#Master_Boot_Record_(MBR)_specific_instructions>)). Don't forget to [grub-mkconfig](https://wiki.archlinux.org/title/GRUB#Generate_the_main_configuration_file).
          - Rootfs-style installers may get annoyed here due to [/etc/vconsole.conf KEYMAP=us being missing](https://wiki.archlinux.org/title/Linux_console/Keyboard_configuration#Persistent_configuration) or linux not being installed
          ```bash
          ...
          grub-mkconfig -o /boot/grub/grub.cfg
          ```
    - Consider [password protection of GRUB menu](https://wiki.gentoo.org/wiki/GRUB#Password_protection_of_GRUB_menu).
...
- Gentoo: Organize make.conf
  ...
  - Consider using [sets](https://wiki.gentoo.org/wiki//etc/portage/sets).
- Install a <tooltip>desktop environment<tip-after>a full-fledged desktop with a taskbar, startmenu, file manager, etc. <br />- GNOME<br />- KDE Plasma Desktop<br />- XFCE4</tip-after></tooltip> or <tooltip class="whitespace-nowrap">window manager<tip-after class="whitespace-normal">a minimal GUI that only handles app windows, leaving the startmenu and others to user setup<br />- Hyprland<br />- Sway</tip-after></tooltip>
  ...
  - [Get fonts](https://wiki.archlinux.org/title/Fonts#Families), such as [Source Han Sans](https://packages.gentoo.org/packages/media-fonts/source-han-sans) for languages, [ancient fonts](https://aur.archlinux.org/packages/ttf-ancient-fonts) for cuneiform, or Noto Sans for large Unicode coverage
- Set up sound.
  - PipeWire ([Arch](https://wiki.archlinux.org/title/PipeWire), [Gentoo](https://wiki.gentoo.org/wiki/PipeWire)) is newer and backwards compatible with PulseAudio clients (e.g. Firefox) via pipewire-pulse.
    - Gentoo: be liberal with these USE flags.
    - Don't forget to [start the user unit for pipewire-pulse.service.](https://wiki.archlinux.org/title/PipeWire#PulseAudio_clients)
      ```bash
      # Arch
      systemctl enable --now --user pipewire-pulse
      ```
    - If using a WM, [pavucontrol](https://wiki.archlinux.org/title/PipeWire#Profiles) may be useful.
  - PulseAudio ([Arch](https://wiki.archlinux.org/title/PulseAudio), [Gentoo](https://wiki.gentoo.org/wiki/PulseAudio)) is older.
- GPUs may require further configuration, especially with [Intel](https://wiki.archlinux.org/title/Intel_graphics)/[Nvidia](https://wiki.archlinux.org/title/NVIDIA).

  - For Intel/Nvidia hybrids, it is recommended ([source](https://wiki.archlinux.org/title/NVIDIA_Optimus#Available_methods)) to use [PRIME](https://wiki.archlinux.org/title/PRIME)
    - Arch Linux's [/usr/bin/prime-run](https://packages.gentoo.org/packages/x11-misc/prime-run) utility is just a shell wrapper
      [to set these environment variables:](https://wiki.gentoo.org/wiki/Hybrid_graphics#Choosing_when_to_use_the_Nvidia_dGPU)
      ```bash
      #!/bin/bash
      ...
      ```
      The nouveau driver's environment variable is simply [DRI_PRIME=1](https://wiki.archlinux.org/title/PRIME#For_open_source_drivers_-_PRIME).
  - CUDA requires the proprietary Nvidia driver ([source](https://wiki.archlinux.org/title/GPGPU#CUDA)).
  - Nvidia implements its own kernel mode setting ([source](https://wiki.archlinux.org/title/Kernel_mode_setting)).
    If you remove the kms HOOK
    [as shown in the Nvidia guide](https://wiki.archlinux.org/title/NVIDIA#Installation),
    you may need to manually set up
    [early or late KMS start](https://wiki.archlinux.org/title/Kernel_mode_setting#Late_KMS_start),
    such as for your iGPU, to avoid errors such as
    [Electron error code 512](https://bbs.archlinux.org/viewtopic.php?id=298254#p2188264).

    > Add the required module... to the initramfs configuration file

    See
    [ArchWiki: mkinitcpio § MODULES](https://wiki.gentoo.org/wiki/Dracut#Adding_modules)
    or [Gentoo: Dracut § Adding modules](https://wiki.gentoo.org/wiki/Dracut#Adding_modules).

  - Make sure your GPU is
    [actually helping your computer run](https://wiki.archlinux.org/title/Hardware_video_acceleration)
    via [glxinfo](https://wiki.archlinux.org/title/PRIME#PRIME_render_offload),
    or [lsmod | grep \<driver name\>](https://wiki.gentoo.org/wiki/NVIDIA/nvidia-drivers#Drivers),
    or a relevant monitoring tool ([Intel](https://wiki.archlinux.org/title/Intel_graphics#Monitoring), [Nvidia](https://wiki.archlinux.org/title/NVIDIA/Tips_and_tricks#nvidia-smi), [AMD](https://wiki.archlinux.org/title/AMDGPU#Monitoring))

- Manage flags and [environment variables](https://wiki.archlinux.org/title/Environment_variables#Defining_variables)
  - Example: Firefox on X11 may need the env var
    [MOZ_USE_XINPUT2=1](https://wiki.archlinux.org/title/Firefox#Touchscreen_gestures_and_pixel-perfect_trackpad_scrolling)
  - Example: Electron apps
    [might run on XWayland, not Wayland](https://wiki.archlinux.org/title/Wayland#Configuration_file)

## 6.5 fluffy tweaks

- Make the CLI easier with bash-completions or zsh+[ohmyzsh](https://aur.archlinux.org/packages/oh-my-zsh-git).
...
- Install and enable the [tlp](https://wiki.archlinux.org/title/Tlp) power management daemon

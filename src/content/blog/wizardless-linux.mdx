...
- GPUs may require further configuration, especially with [Intel](https://wiki.archlinux.org/title/Intel_graphics)/[Nvidia](https://wiki.archlinux.org/title/NVIDIA).
  - For Intel/Nvidia hybrids, it is recommended ([source](https://wiki.archlinux.org/title/NVIDIA_Optimus#Available_methods)) to use [PRIME](https://wiki.archlinux.org/title/PRIME)
    - Arch Linux's [/usr/bin/prime-run](https://packages.gentoo.org/packages/x11-misc/prime-run) utility is just a shell wrapper [to set these environment variables:](https://wiki.gentoo.org/wiki/Hybrid_graphics#Choosing_when_to_use_the_Nvidia_dGPU)
      ```bash
      #!/bin/bash
  ...
  - [CUDA requires the proprietary Nvidia driver](https://wiki.archlinux.org/title/GPGPU#CUDA).
  - To run Nvidia, the loading of the nouveau graphics module will need to be stopped, either by [stopping "kernel mode setting" loading the module from the initramfs stage](https://wiki.archlinux.org/title/NVIDIA#Installation) (may cause bugs such as Electron exit_code=512 GPU errors) or by using a blacklist:
    ```bash
    # /usr/lib/modprobe.d/nvidia-utils.conf
    blacklist nouveau
    # NOTE: This is included in Arch's nvidia-utils package
    ```
  - Make sure your GPU is [actually helping your computer run](https://wiki.archlinux.org/title/Hardware_video_acceleration)
...
- Manage flags and [environment variables](https://wiki.archlinux.org/title/Environment_variables#Defining_variables)
  - Example: Firefox may need the env var [MOZ_USE_XINPUT2=1](https://wiki.archlinux.org/title/Firefox#Touchscreen_gestures_and_pixel-perfect_trackpad_scrolling)
  - Example: Electron apps [might run on XWayland, not Wayland](https://wiki.archlinux.org/title/Wayland#Configuration_file)
